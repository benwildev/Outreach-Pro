...
// Build a complete email→index map by injecting the ListAccounts fetch INTO a Gmail tab.
// The service worker cannot include Google's GAIA session cookies in cross-origin fetches,
// but an injected script running inside the Gmail page can — it uses the page's cookies
...
async function fetchGmailAccountMap() {
  ...
  try {
    // Prefer an already-open Gmail tab at a numeric /u/N/ URL.
    ...
    // Fall back to opening a silent background tab if no Gmail tab is available.
    if (!targetTabId) {
    ...
    // Run the ListAccounts fetch inside the Gmail page context (cookies attached).
    ...
    console.log("[Leads Extension] fetchGmailAccountMap: resolved", Object.keys(map).length, "account(s)");
    return map;
  } finally {
    if (tempTabId) {
...
async function fetchSendQueue(limit, campaignId, gmailAcct) {
  ...
  const normalizedGmailAcct = String(gmailAcct || "").trim();
  if (normalizedGmailAcct) {
    params.set("gmailAcct", normalizedGmailAcct);
  }
...
async function fetchFollowupQueue(limit, campaignId, step, fu1GmailOverride, gmailAcct) {
  const maxLeads = normalizeBulkLimit(limit);
  const params = new URLSearchParams();
  params.set("limit", String(maxLeads));
  const normalizedCampaignId = String(campaignId || "").trim();
...
async function runBulkAutomationQueue() {
  ...
  try {
    ...
    if (!bulkAutomationState.stopRequested && bulkAutomationState.followupEnabled) {
      ...
      const followupQueue = await fetchFollowupQueue(bulkAutomationState.limit, bulkAutomationState.campaignId, stepFilter, bulkAutomationState.fu1GmailOverride, bulkAutomationState.gmailAcct);
...
async function handleStartBulkAutomation(data) {
  ...
  const gmailAcct = data && data.gmailAcct ? String(data.gmailAcct).trim() : "";
  const queue = isFollowupOnly ? [] : await fetchSendQueue(limit, campaignId, gmailAcct);
  ...
  bulkAutomationState.gmailAcct = gmailAcct;
